...
def tokenized_tqa_gen_truthfulqa(dataset, tokenizer):
    all_prompts = []
    for i in range(len(dataset)): 
        # Get question and answers from dataset
        question = dataset.iloc[i]['Type']  # Assuming "question" corresponds to this column

        # Handle correct answers
        correct_answers = dataset.iloc[i]['Correct Answers'].split(';')  # assuming multiple answers are separated by ";"
        for answer in correct_answers: 
            prompt = format_truthfulqa_Shakespeare(question, answer)
            prompt_tokenized = tokenizer(prompt, return_tensors='pt').input_ids
            all_prompts.append(prompt_tokenized)
    return all_prompts

def tokenized_tqa_gen_alpaca(dataset, tokenizer):
    all_prompts = []
    for i in range(len(dataset)): 
        instruction = dataset[i]['instruction']
        inputs =  dataset[i]['input']
        output =  dataset[i]['output']
        prompt = "Please follow the instruction to give the response:### Instruction:\n"+instruction+inputs+"### Response:\n"+output
        prompt_tokenized = tokenizer(prompt, return_tensors='pt').input_ids
        all_prompts.append(prompt_tokenized)
...
def train_probes(seed, train_set_idxs, val_set_idxs, separated_head_wise_activations, separated_labels, num_layers, num_heads):
    ...
    y_train0 = np.concatenate([separated_labels[i] for i in train_set_idxs], axis = 0)
    y_val0 = np.concatenate([separated_labels[i] for i in val_set_idxs], axis = 0)
    import ipdb
    ...
    for layer in tqdm(range(num_layers)): 
        for head in range(num_heads): 
            ...
            if np.any(np.isnan(X_train)):
                # Copy the last element of probes (which is clf) into clf
                clf = probes[-1] if len(probes) > 0 else LogisticRegression(random_state=seed, max_iter=1000)

                # Append clf to probes
                probes.append(clf)
                continue
            '''
            if np.any(np.isnan(X_train)):
            ...
            if np.any(np.isnan(X_val)):
                ...
                y_val_clean = y_val[~np.isnan(X_val).any(axis=1)]
                X_val =  X_val_clean
                y_val = y_val_clean
...
class CustomTrainingArguments(TrainingArguments):
    reg_lambda: float = field(
        default=0.1,
        metadata={"help": "Regularization coefficient for mask prior alignment"}
    )
    align_lambda: float = field(
        default=0.1,
        metadata={"help": "Regularization coefficient for initial prior alignment"}
    )
